...
type
  ...
  utUSSurvIN,   // U.S. Surveyorґs Inch
  utUSSurvFT,   // U.S. Surveyorґs Foot
  utUSSurvYard, // U.S. Surveyorґs Yard
  utUSSurvChain,// U.S. Surveyorґs Chain
  utUSSurvSM,   // U.S. Surveyorґs Statute Mile
...
begin
 ...
 if (aValue[1] in ['W','S','-','ю','Ю','з','З']) then result:=-result;
end;
...
begin
 ...
 case DegType of
  dtDegLat      : if aRadValue<0 then result:='ю'+result else result:='с'+result;
  dtDegLong     : if aRadValue<0 then result:='з'+result else result:='в'+result;
...
begin
 ...
 case index of
  ...
  014: InitChamberlin(PJ,Chamb.PT[0].P,Chamb.PT[1].P,Chamb.PT[2].P);    // не отлажена
...
begin
  ...
  begin
    if n>2 then break;
    if sQuad[i] in ['А'..'Г'] then
    begin
     K[n]:=ord(sQuad[i])-ord('А');
     inc(n);
    end else
    if sQuad[i] in ['а'..'г'] then
    begin
